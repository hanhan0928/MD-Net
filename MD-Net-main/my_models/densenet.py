...
class MyDensenet (nn.Module):
    ...
    def __init__(self, densenet, num_class, neurons_reducer_block=256, freeze_conv=False, p_dropout=0.5,
        ...
        if comb_method is not None:
            ...
            if comb_method == 'metablock':
            ...
            elif comb_method == 'netblock':
                if isinstance(comb_config, int):
                    self.comb_feat_maps = 16
                    self.comb_feat_maps1 = 32
                    self.comb = NetBlock(comb_config, 64, 16, self.comb_feat_maps1, comb_config)
                elif isinstance(comb_config, list):
                    self.comb = NetBlock(comb_config[0], comb_config[1], comb_config[2], self.comb_feat_maps1,
                                         comb_config[1])
                    self.comb_feat_maps = comb_config[2]
                    self.comb_feat_maps1 = comb_config[0]
                else:
                    raise Exception(
                        "comb_config must be a list or int to define the number of feat maps and the metadata")
                ...
                if isinstance(comb_config, int):
                    self.comb_feat_maps = 16
                    self.comb = MetaNet(comb_config, 64, 16)
                elif isinstance(comb_config, list):
        ...
        # Here comes the extra information (if appliFcable)
        if neurons_reducer_block > 0:
    ...
    def forward(self, img, meta_data=None):
        ...
        # 添加的部分
        elif isinstance(self.comb, NetBlock):
            x = x.view(x.size(0), self.comb_feat_maps, 8, 8).squeeze(-1)
            x1 = x.view(x.size(0), self.comb_feat_maps1, 32, -1).squeeze(-1)  # getting the feature maps
            x = self.comb(x, meta_data.float(), x1, meta_data.float())  # applying metanet
            x = x.view(x.size(0), -1)  # flatting
            if self.reducer_block is not None:
                x = self.reducer_block(x)  # feat reducer block
